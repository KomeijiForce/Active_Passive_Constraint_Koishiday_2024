...
from prompt import fill_in_query_discrimination_template, fill_in_brief_nli_discrimination_template
...
def get_relevance_discriminator(character, statement_query_relevance_dataset, relevance_finetune_epoch):
    ...
    if os.path.isdir(f'discriminators/deberta-v3-large-{character}-relevance'):
        ...
        return Classifier(model_name=f'discriminators/deberta-v3-large-{character}-relevance', device='cuda:0', num_labels=2)

    relevance_discriminator = Classifier(model_name='microsoft/deberta-v3-large', device='cuda:0', num_labels=2)

    statement_query_relevance_dataset_for_finetune = [{"text": fill_in_query_discrimination_template(data["character"], data["statement"], data["query"]), 
    ...
    relevance_discriminator.tok.save_pretrained(f'discriminators/deberta-v3-large-{character}-relevance')
    relevance_discriminator.classifier.save_pretrained(f'discriminators/deberta-v3-large-{character}-relevance')
...
def get_nli_discriminator(character, statement_to_response_nli_v2_dataset, nli_finetune_epoch):
    ...
    if os.path.isdir(f'discriminators/deberta-v3-large-{character}-nli'):
        ...
        return Classifier(model_name=f'discriminators/deberta-v3-large-{character}-nli', device='cuda:0', num_labels=3)

    nli_discriminator = Classifier(model_name='microsoft/deberta-v3-large', device='cuda:0', num_labels=3)

    statement_to_response_nli_v2_dataset_for_finetune = [{"text": fill_in_brief_nli_discrimination_template(data["character"], data["statement"], data["query"], data["response"]), 
    ...
    nli_discriminator.tok.save_pretrained(f'discriminators/deberta-v3-large-{character}-nli')
    nli_discriminator.classifier.save_pretrained(f'discriminators/deberta-v3-large-{character}-nli')
